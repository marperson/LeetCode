...
class Solution:
    ...
    def maj(self,arr):
        ...
        for x in range(lenArr):
            count = 0
            # This assertion is to avoid non necessary iteration
            if majority.count(arr[x])==0:
                for y in range(lenArr):
                    if arr[x] == arr[y]:
                        count +=1
                
                if count > lenArr/2:
                    majority.append(arr[x])
...
s = Solution()
print (s.majorityElement([1,2,3,4,5,6,46,6,4,4,4,4,4,6,64,4,4,4,4,4,4,4,6,6,6]))
